...
%% data import
table = readtable("obj_waveguide.txt");
...
% plot(frequency, 20*log10(abs(s_11)), 'DisplayName', 'abs$(s11)$', 'LineWidth', 1.2)
% plot(frequency, angle(s_21)*180/pi, 'DisplayName', 'angle$(s21)$', 'LineWidth', 1.2)
% xlabel('$f$, Hz', 'Interpreter','latex', 'FontSize', 13)
% ylabel('Absolute value, dB', 'Interpreter','latex', 'FontSize', 13)
% legend('Interpreter','latex', 'FontSize', 13)
% grid on

% % % % % % % % % % Y = fft(s_21);
% % % % % % % % % % time = (0:(1601-1))*1/(4*10.^9)/1607;
% % % % % % % % % % 
% % % % % % % % % % new_time = time(1:1200);
% % % % % % % % % % S21_fft = fft(Y(1:1200));
% % % % % % % % % % 
% % % % % % % % % % plot(frequency(1:1200), abs(S21_fft))
% % % % % % % % % % grid on

%% Для deembadding

table_thru = readtable("kvp_x2.txt");


ReS11_kvp = table_thru{:, 2};% ./ table_free{:, 2};
ImS11_kvp = table_thru{:, 3};% ./ table_free{:, 3};
s_11_kvp = ReS11_kvp + 1i * ImS11_kvp;

ReS21_kvp = table_thru{:, 4};% ./ table_free{:, 4};
ImS21_kvp = table_thru{:, 5};% ./ table_free{:, 5};
s_21_kvp = ReS21_kvp + 1i * ImS21_kvp;

ReS12_kvp = table_thru{:, 6};% ./ table_free{:, 6};
ImS12_kvp = table_thru{:, 7};% ./ table_free{:, 7};
s_12_kvp = ReS12_kvp + 1i * ImS12_kvp;

ReS22_kvp = table_thru{:, 8};% ./ table_free{:, 8};
ImS22_kvp = table_thru{:, 9};% ./ table_free{:, 9};
s_22_kvp = ReS22_kvp + 1i * ImS22_kvp;

% plot(frequency, 20*log10(abs(s_21_kvp)), 'DisplayName', '$|s11(kvp)|$', 'LineWidth', 1.2)
% plot(frequency, unwrap(angle(s_21_kvp)*180/pi), 'DisplayName', 'angle$(s12_kvp)$', 'LineWidth', 1.2)
% xlabel('$f$, Hz', 'Interpreter','latex', 'FontSize', 13)
% ylabel('Phase, deg', 'Interpreter','latex', 'FontSize', 13)
% ylabel('Absolute value, dB', 'Interpreter','latex', 'FontSize', 13)
% legend('Interpreter','latex', 'FontSize', 13)
% grid on

phi = -1/2 * unwrap(angle(s_21_kvp));


%% deembading

R1 = exp(-1i*L1*2*pi/c .* frequency .* (1 - (f_0 ./ frequency).^2).^(1/2)) .* exp(-1i .* phi);
R2 = exp(-1i*L2*2*pi/c .* frequency .* (1 - (f_0 ./ frequency).^2).^(1/2)) .* exp(-1i .* phi);
